...
knitr::opts_chunk$set(out.width = "65%", echo = TRUE)
```
...
+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+---------------------------------+------------------------------------------------------------------------------------------------------+
| H13: In addition to H6, we hypothesize the existence of a causal pathway leading from **social media exposure** to **depression**, which is mediated by **anxiety** and moderated by **age** and **gender.**      | q01, q02, q18_02, anxiety_index | [@Bendau2020; @Rasmussen2020; @Wheaton2021; @Vannucci2017; @Mertens2020]                             |
+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+---------------------------------+------------------------------------------------------------------------------------------------------+
...
# to construct an anxiety/concern index with factor analysis (to use for 
...
data <- data %>% 
  transmute(id = RespondentID,
             q01_gender = as_factor(q01),
             q02_age = q02,
             q02_age_groups = as_factor(Q4_AGE_r, ordered = TRUE),
             q03_relationship_type = as_factor(q03),
             q04_children = as_factor(q04),
             q11_education = as_factor(q11),
             q18_02_soc_media_info = as_factor(replace_na(q18_02, 0)),
             q20_public_info = as_factor(q20),
             q34_02_face_mask = as_factor(q34_02),
             q34_07_hand_washing = as_factor(q34_07),
             q35_01_contact_close_family = q35_01,
             q35_03_contact_friends = q35_03,
             q36_econ_worry = as_factor(q36),
             q38_alcohol = as_factor(q38),
             q40_smoking = as_factor(q40),
             q42_sport = as_factor(q42),
             q47_self_reporting_health = as_factor(q47, ordered = TRUE),
             q48_chronic_illness = as_factor(q48),
             q49_health_limitations = as_factor(q49, ordered = TRUE),
             q30_anxiety_infection_covid = q30,
             q31_anxiety_infection_friends = q31,
             q33_01_anxiety_situation = q33_01,
             q33_02_anxiety_low_control = q33_02,
             q33_03_anxiety_survival_covid = q33_03,
             q33_04_anxiety_change_employment = q33_04,
             q33_05_anxiety_infecting_others = q33_05,
             PHQ8 = PHQ8,
             q50_comment = q50)

levels(data$q01_gender) <- list(female = "Žena", 
                         male = "Muž")
levels(data$q03_relationship_type) <- list(single = "Svobodný/Svobodná", 
                         relationship = "V partnerském vztahu", 
                         married = "Ženatý/Vdaná", 
                         divorced = "Rozvedený/Rozvedená", 
                         widowed = "Vdovec/Vdova")
levels(data$q04_children) <- list(yes = "Ano", no = "Ne")
levels(data$q11_education) <- list(unfin_element = "Nedokončené základní", 
...
levels(data$q20_public_info) <- list(yes = "Ano", no = "Ne", do_not_know = "Nevím")
levels(data$q34_02_face_mask) <- list(yes = "ano", no = "ne")
levels(data$q34_07_hand_washing) <- list(yes = "ano", no = "ne")
levels(data$q36_econ_worry) <- list(limited = "Vůbec žádný nebo jen nepatrný", 
                                    serious = "Ano, celkem významný", 
                                    very_serious = "Ano, velmi vážný")
levels(data$q38_alcohol) <- list(yes = "Ano", no = "Ne")
levels(data$q40_smoking) <- list(yes = "Ano", no = "Ne")
levels(data$q42_sport) <- list(yes = "Ano", no = "Ne")
levels(data$q47_self_reporting_health) <- list(excellent = "Velmi dobrý",
                         good = "Dobrý", 
                         neutral = "Ani dobrý ani špatný", 
                         bad = "Špatný", 
                         very_bad = "Velmi špatný")
levels(data$q48_chronic_illness) <- list(yes = "Ano", no = "Ne")
levels(data$q49_health_limitations) <- list(limits = "Velmi omezuje", 
                         partially_limits = "Částečně omezuje", 
                         no_limits = "Vůbec neomezuje")

kableExtra::kbl(head(data), caption = "The overview of the structure of the dataset and its key variables") %>%
...
### Demographic characteristics results table {.tabset}
...
data_corr <- data_corr %>% 
              transmute(q01_gender = q01, 
                        ...
                        q04_children = q04,
                        q11_education = q11,
                        q18_02_soc_media_info = replace_na(q18_02, 0),
                        q20_public_info = q20,
...
## Stepwise regression models, step 1: algorithmic selection
...
Using AIC-ranked stepwise selection, the algorithm arrives at 13-predictor model.

```{r automatic stepwise regression model AIC-selected, comment = ""}
...
data_linreg <- data %>% dplyr::select(-c(id, 
                                         q02_age_groups,
...
```{r automatic stepwise regression model BIC-selected, comment = ""}
...
## Stepwise regression models, step 2: final AIC and BIC-driven model

In order to allow direct comparison with the manually-selected model, we input the chosen models (based on the AIC and BIC criterion) from the previous step into the `linreg()` function of the `jmv` package. The first, simpler model has the 7 predictors from the BIC-selected model. The second, more complex model, has 6 additional variables from AIC-selected stepwise model (to a total of 13).

```{r stepwise selection-built linear model, comment = ""}
...
# Anxiety factor as a mediator for depression

## Creation of the Covid-related Anxiety index, step 1: overview of survey items
...
Unlike as is in the case of PHQ-8 index as a measure of depression, this survey does not have a standardized measure of anxiety. We therefore try to proceed inductively, using Covid-19-related survey items that could represent anxiety.

Therefore, in this section, we aim to construct an anxiety index from several survey items using factor analysis. As a first step, we select the survey items, which should be the manifestation of the latent factor of anxiety.
...
## Creation of the Covid-related Anxiety index, step 2: anxiety items descriptives and pre-processing
...
```{r anxiety items descriptives, comment = ""}
jmv::descriptives(
    data = data,
...
# We also split the sample into two halfs. The "training" half, on which we 
...
## Creation of the Covid-related Anxiety index, step 3: Exploratory Factor Analysis of anxiety items
...
The result is therefore a one-factor construct, which includes all of the variables, except for the `q33_04_anxiety_change_employment`, which does not seem to be a good fit for the manifestation of anxiety within this group of variables. We will exclude this variable in the next step.

```{r anxiety items Exploratory Factor Analysis, comment = ""}
...
## Creation of the Covid-related Anxiety index, step 3: Reliability Analysis of the index items
...
```{r anxiety index reliability analysis, comment = ""}
...
## Creation of the Covid-related Anxiety index, step 4: Confirmatory Factor Analysis of the index items
...
```{r anxiety index Confirmatory Factor Analysis, comment = ""}
...
## Creation of Covid-related Anxiety Index , step 5: creation and descriptives
...
```{r anxiety index creation and descriptives, comment = ""}
# Creating the Covid-related concern/anxiety index, consisting of the average of 
# the values of the multiple variables selected through factor analysis to
# represent the underlying construct of anxiety.
...
#To summarize the anxiety_index variable, we use the descriptives() 
# function from the jmv package.
...
# Simplified age-moderated & anxiety-mediated model

## Moderated mediation model conceptual diagram and pre-processing

To explore our hypothesized pathway (see H13) between social media exposure and depression, partially mediated by anxiety and moderated by age (which is presumed to influence both the social media exposure and the depression pathway), we conduct a mediation-moderation analysis using the `lavaan` package, conceptually structured as a [Hayes model nr. 8.](https://osf.io/29c8p/download)

```{r path diagram Hayes model 8}
# Conceptual diagram
labels_H8 <- list(X = "Social Media",M = "Anxiety",Y = "Depression", W = "Age")
pmacroModel(8,labels = labels_H8, xmargin = 0, rady = 0.05, radx = 0.08, ylim = c(0.35,1))
```

```{r mediation pre-processing}
# Before running the model, we need to transform the social media string 
# dummy (yes/no) back to its numeric form.
...
data <- data %>% 
        ...
        mutate(anxiety_index.c = scale(anxiety_index, scale = TRUE),
               q18_02_soc_media_info.c = scale(q18_02_soc_media_info, scale = TRUE),
...
```{r Hayes model 8, warning=FALSE, comment = ""}
...
PHQ8.c~c1*q18_02_soc_media_info.c+c2*q02_age.c+c3*q18_02_soc_media_info.c:q02_age.c+b*anxiety_index.c

#Mean of centered age moderator
q02_age.c ~ q02_age.c.mean*1

#Variance of centered age moderator
q02_age.c ~~ q02_age.c.var*q02_age.c

index.mod.med :=a3*b
indirect :=(a1+a3*q02_age.c.mean)*(b)
direct :=c1+c3*q02_age.c.mean
total := direct + indirect
prop.mediated := indirect / total
CE.XonM :=a1+a3*q02_age.c.mean

CE.XonM.below :=a1+a3*(q02_age.c.mean-sqrt(q02_age.c.var))
CE.XonM.above :=a1+a3*(q02_age.c.mean+sqrt(q02_age.c.var))
indirect.below :=(a1+a3*(q02_age.c.mean-sqrt(q02_age.c.var)))*(b)
indirect.above :=(a1+a3*(q02_age.c.mean+sqrt(q02_age.c.var)))*(b)
direct.below:=c1+c3*(q02_age.c.mean-sqrt(q02_age.c.var))
direct.above:=c1+c3*(q02_age.c.mean+sqrt(q02_age.c.var))
...
kableExtra::kbl(estimatesTable(fit_mod), caption = "Table of model estimates") %>%
      kableExtra::kable_classic(lightable_options = c("striped")) %>%
      kableExtra::scroll_box(width = "830px", height = "100%")
```
...
## Word Clouds of all textual responses, step 1: pre-processing and tokens Word Cloud
...
# Should the above link become obsolete, alternative source can be reached using "stopwords" library:
# stop_words_cz <- as_tibble_col(stopwords::stopwords("cs", source = "stopwords-iso"), column_name = "word")
...
#  can also remove unique stop words that are still present (optional step)
...
## Word Clouds of all textual responses, step 2: pre-processing and lemma Word Cloud
...
## Sentiment analysis of qualitative responses, step 1: pre-processing and Word Cloud with positive valency
...
# Next, we create tidy tibble with tokens created in the previous section and we use inner_join 
# function to separately save only the tokens with positive and negative valency
...
## Sentiment analysis of qualitative responses, step 2: Word Cloud with negative valency
