---
title: "Predictors of Depression during the Covid-19, Czech sample report v1.0"
...
data <- data %>% 
        ...
        mutate(q20 = haven::as_factor(q20)) %>%
        mutate(q34_02 = haven::as_factor(q34_02)) %>%
        mutate(q34_07 = haven::as_factor(q34_07)) %>%
        mutate(q36 = haven::as_factor(q36)) %>%
...
levels(data$q20) <- list(yes = "Ano", no = "Ne", do_not_know = "Nevím")
levels(data$q34_02) <- list(yes = "ano", no = "ne")
levels(data$q34_07) <- list(yes = "ano", no = "ne")
levels(data$q36) <- list(limited = "Vůbec žádný nebo jen nepatrný", serious = "Ano, celkem významný", very_serious = "Ano, velmi vážný")
...
data <- data %>% 
        ...
        rename(q20_public_info = q20) %>%
        rename(q34_02_face_mask = q34_02) %>%
        rename(q34_07_hand_washing = q34_07) %>%
        rename(q36_econ_worry = q36) %>%
...
# Normality transformation: finding lambda for entire model that includes PHQ8 as a dependent variable
...
```{r sample demography descriptives, comment = ""}
...
corrplot(cor(data_corr, method = "spearman", use = "complete.obs"), method = "circle", title = "Correlation Matrix - Spearman Coefficient", type = "lower", p.mat = res1$p, sig.level = .05, mar = c(0,0,1,0))
```
...
#### d. Stepwise regression models, step 2: final AIC and BIC-driven model
...
#### a. Moderated mediation model conceptual diagram and pre-processing
...
```{r path diagram Hayes model 8, fig.height=5, fig.width=9}
# Conceptual diagram
...
#Before running the model, we need to transform the social media string dummy (yes/no) back to its numeric form.
...
data <- data %>% 
        mutate(PHQ8.c = scale(PHQ8_t, scale = TRUE)) %>% 
        mutate(q02_age.c = scale(q02_age, scale = TRUE)) %>%
...
#### b. Moderated mediation model specification and results
...
estimatesTable(fit_mod)
```

## 6. Visualizing the qualitative responses using Word Clouds

Part of the survey, `q50`, was dedicated to the comments of the respondents on their situation. To visualize this textual data, we use two pairs of two word clouds. Unfortunately, this survey item was used only in the Czech version of the survey.
