...
First model (from the set of 5) uses only the demographic characteristics as predictors. Second model adds the effect of the social media consumption. Third model adds lifestyle variables, such as alcohol, smoking, sport and social contacts. The fourth model further adds the variables related to self-rated health quality. Finally, the fifth model controls for the age by adding it as a covariate. The performance of each model could be seen in the output below.
...
To explore our hypothesized pathway (see H13) between social media exposure and depression, partially mediated by anxiety, we conduct a mediation analysis.
